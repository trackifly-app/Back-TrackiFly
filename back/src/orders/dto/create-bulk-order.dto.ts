...
/**
 * DTO para un pedido individual dentro de una petición bulk
 * Hereda de CreateOrderDto pero añade validaciones específicas para bulk
 */
...
/**
 * DTO para creación masiva de pedidos
 * Agrupa múltiples órdenes para ser procesadas en una sola transacción
 */
export class CreateBulkOrderDto {
  @IsNotEmpty({ message: 'El ID de la company es requerido' })
  @IsUUID('4', { message: 'El ID de la company debe ser un UUID válido' })
  companyId: string;

  @IsArray({ message: 'Los pedidos deben ser un array' })
  @ValidateNested({ each: true })
  @Type(() => BulkOrderItemDto)
  @ArrayMinSize(1, { message: 'Debes proporcionar al menos 1 pedido' })
  @ArrayMaxSize(1000, {
    message: 'No puedes crear más de 1000 pedidos en una sola petición',
  })
  orders: BulkOrderItemDto[];

  @IsOptional()
  @IsBoolean({ message: 'continueOnError debe ser booleano' })
